...
const int EVENT_REQUEST = 0xFF;
const int EVENT_RESPONSE = 0xFE;
const int HEARTBEAT = 0xFD;
const int READY = 0xFC;  
...
void addListener(NintacoAPI* api, void* listener, int32_t eventType);
int addListenerObj(NintacoAPI* api, void* listener, 
      int32_t eventType);
int addListenerObject(NintacoAPI* api, void* listener,
    int32_t eventType, void* listenerObject);
void closeSocketConnect(NintacoAPI* api);
void deleteAccessPoint(AccessPoint* accessPoint);
void deleteDataInputStream(DataInputStream* in);
void deleteDataOutputStream(DataOutputStream* out);
void deleteIntObjectEntry(IntObjectEntry* map);
void deleteNintacoAPI(NintacoAPI* api);
void deleteObjectIntEntry(ObjectIntEntry* map);
void deleteScanlineCyclePoint(ScanlineCyclePoint* scanlineCyclePoint);
void deleteScanlinePoint(ScanlinePoint* scanlinePoint); 
void fireDeactivated(NintacoAPI* api);
void fireStatusChanged(NintacoAPI* api, char* message, ...);
void flush(DataOutputStream* out);
IntObjectEntry* IntObjectEntryClone(IntObjectEntry* map);
IntObjectEntry* IntObjectEntryGet(IntObjectEntry* map, int32_t key);
IntObjectEntry* IntObjectEntryPut(IntObjectEntry* map, int32_t key, 
    void* value);
IntObjectEntry* IntObjectEntryRemove(IntObjectEntry* map, 
    IntObjectEntry* entry);
AccessPoint* newAccessPoint(AccessPointListener listener, 
    int type, int minAddress, int maxAddress, int bank);
DataInputStream* newDataInputStream(int fileDescriptor);
DataOutputStream* newDataOutputStream(int fileDescriptor);
IntObjectEntry* newIntObjectEntry(int32_t key, void* value);
ObjectIntEntry* newObjectIntEntry(void* key, int32_t value);
ScanlineCyclePoint* newScanlineCyclePoint(ScanlineCycleListener listener, 
    int scanline, int scanlineCycle);
ScanlinePoint* newScanlinePoint(ScanlineListener listener, int scanline);
ObjectIntEntry* ObjectIntEntryClone(ObjectIntEntry* map);
ObjectIntEntry* ObjectIntEntryGet(ObjectIntEntry* map, void* key);
ObjectIntEntry* ObjectIntEntryPut(ObjectIntEntry* map, void* key, 
    int32_t value);
ObjectIntEntry* ObjectIntEntryRemove(ObjectIntEntry* map, 
    ObjectIntEntry* entry);
void openSocketConnection(NintacoAPI* api);
void probeEvents(NintacoAPI* api);
bool readBoolean(DataInputStream* in);
unsigned char readByte(DataInputStream* in);
char readChar(DataInputStream* in);
int readCharArray(DataInputStream* in, char* array, int length);
char** readDynamicStringArray(DataInputStream* in);
int32_t readInt(DataInputStream* in);
int readIntArray(DataInputStream* in, int32_t* array, int length);  
char* readString(DataInputStream* in);
int readStringArray(DataInputStream* in, char** array, int arrayLength);
void removeListener(NintacoAPI* api, void* listener, int32_t eventType, 
    int32_t methodValue);
int removeListenerObject(NintacoAPI* api, void* listener, 
    int32_t eventType);
void sendListener(NintacoAPI* api, int32_t listenerID, int32_t eventType, 
    void* listenerObject);
void sendListeners(NintacoAPI* api);
void sendReady(NintacoAPI* api);
void sleep_ms(int milliseconds);
void writeBoolean(DataOutputStream* out, bool value);
void writeByte(DataOutputStream* out, unsigned char value);
void writeChar(DataOutputStream* out, char value);
void writeCharArray(DataOutputStream* out, char* array, int length);
void writeInt(DataOutputStream* out, int32_t value);
void writeIntArray(DataOutputStream* out, int32_t* array, int length);
void writeString(DataOutputStream* out, char* value);
void writeStringArray(DataOutputStream* out, char** array, int length);
